...
file, corresponding to one CHANGELOG entry. Each plan is independent from
other plans — it does not reference or repeat the content of previous
plans — but it MUST follow the foundational architecture decisions already
locked in the feature's `architecture.md`, and it must NEVER introduce a
technical decision that goes beyond what `spec.md` describes.
...
6. Write `plans/plan-XXX.md` (XXX = the matching CHANGELOG entry ID),
   containing:
   - Which Requirement ID(s) this plan implements
